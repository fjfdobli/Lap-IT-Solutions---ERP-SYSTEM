...
const mainNavItems = [
  ...
  { 
    ...
    description: 'Desktop access control'
  },
  { 
    to: '/devices', 
    icon: Monitor, 
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (newUnreadCount > previousUnreadCountRef.current && previousUnreadCountRef.current > 0) {
          const newNotifications = response.data.notifications.filter(n => !n.isRead)
          if (newNotifications.length > 0 && Notification.permission === 'granted') {
            new Notification('New Notification', {
              body: newNotifications[0]?.title || 'You have new notifications',
              icon: '/images/lap_it_no-bg.png',
            })
          }
        }
      ...
      <div className={cn(
        ...
        {!collapsed && (
          <div className="flex flex-col">
            ...
            <span className="text-xs text-sidebar-foreground/60">Super Admin Portal</span>
          </div>
        )}
      </div>
          ...
          <div className="flex h-full items-center justify-between px-4 lg:px-6">
            ...
            <div className="hidden md:flex items-center flex-1 max-w-md mx-4">
              <div className="relative w-full">
                ...
                <input
                  type="text"
                  placeholder="Search users, roles, devices..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && searchQuery.trim()) {
                      if (location.pathname.includes('/users')) {
                        // Already on users page, the page should handle the search
                  ...
                  <div className="max-h-[400px] overflow-y-auto">
                    {notifications.length === 0 ? (
                      <div className="py-8 text-center">
                        ...
                        <p className="text-xs text-muted-foreground/70 mt-1">You'll see activity updates here</p>
                      </div>
                    ) : (
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-between p-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="text-xs h-8 text-primary hover:text-primary ml-auto"
                          onClick={() => {
