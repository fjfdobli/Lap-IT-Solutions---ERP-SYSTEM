...
import { RotateCcw, XCircle, Package, AlertTriangle, FileX, AlertCircle } from 'lucide-react'
...
const tabs = [
  { id: 'pos_void_header', label: 'Voided Sales', icon: XCircle, description: 'Cancelled transaction records' },
  { id: 'pos_void_details', label: 'Voided Items', icon: FileX, description: 'Items from voided transactions' },
  { id: 'pos_return_header', label: 'Sales Returns', icon: RotateCcw, description: 'Customer return transactions' },
  { id: 'pos_return_details', label: 'Return Items', icon: Package, description: 'Products returned by customers' },
]
...
export default function VoidsReturnsPage() {
  ...
  const Icon = posConfig?.icon || AlertTriangle
          ...
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="px-3 py-1.5">
                <RotateCcw className={cn("h-3.5 w-3.5 mr-1.5", posConfig?.textColor)} />
                Adjustments
              </Badge>
            </div>
          </CardHeader>
      ...
      <motion.div variants={itemVariants}>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="pos_void_header" className="mt-0">
            <POSTableViewer 
              tableName="pos_void_header" 
              title="Voided Transactions" 
              description="Cancelled sales transaction records"
              icon={<XCircle className={cn("h-6 w-6", posConfig?.textColor)} />}
            />
          </TabsContent>
          <TabsContent value="pos_void_details" className="mt-0">
            <POSTableViewer 
              tableName="pos_void_details" 
              title="Voided Line Items" 
              description="Items from voided transactions"
              icon={<FileX className={cn("h-6 w-6", posConfig?.textColor)} />}
            />
          </TabsContent>
          <TabsContent value="pos_return_header" className="mt-0">
            <POSTableViewer 
              tableName="pos_return_header" 
              title="Sales Returns" 
              description="Customer return transaction records"
              icon={<RotateCcw className={cn("h-6 w-6", posConfig?.textColor)} />}
            />
          </TabsContent>
          <TabsContent value="pos_return_details" className="mt-0">
            <POSTableViewer 
              tableName="pos_return_details" 
